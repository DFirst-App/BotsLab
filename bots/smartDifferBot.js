(() => {
  class SmartDifferBot {
        ...
        switch (data.msg_type) {
          case 'authorize':
            ...
            if (this.isReconnecting) {
              ...
              this.ui.showStatus('Connected. Starting smart differ strategy...', 'success');
            }
          ...
          case 'proposal_open_contract':
            ...
            break;
          case 'tick':
            // Store last digit from tick for differ strategy
            if (data.tick && data.tick.quote) {
              const tickValue = data.tick.quote.toString();
              this.lastDigit = parseInt(tickValue.slice(-1));
            }
            break;
          default:
            break;
    ...
    queueNextTrade() {
      ...
      this.ui.updateTargets(market, `Differ ${digit}`);

      // Subscribe to ticks for the selected market to get current digit
      this.ws.send(JSON.stringify({
        ticks: market,
        subscribe: 1
      }));

      // Small delay to ensure we have tick data, then send proposal
      setTimeout(() => {
    ...
    getNextDigit() {
      ...
      // Avoid same digit as last trade
  ...
  window.SmartDifferBot = SmartDifferBot;
})();
