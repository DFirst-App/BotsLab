(() => {
  class SimSmartEvenBot {
    ...
    resetState() {
      ...
      this.currentChoice = null; // 'EVEN' or 'ODD'
      this.tradeHistory = [];
    }
    ...
    async start(config) {
      ...
      this.ui.showStatus('Simulation mode: Starting smart even/odd...', 'success');
      this.isRunning = true;
      this.stopRequested = false;
    ...
    queueNextTrade() {
      ...
      // Simulate anti-streak logic: choose opposite of recent pattern
      const choice = Math.random() < 0.5 ? 'EVEN' : 'ODD';
      this.tradeInProgress = true;
      this.currentChoice = choice;
      this.ui.updateTargets(this.symbol, choice);

      const duration = this.simBase.getContractDuration(1);
      this.tradeTimeout = setTimeout(() => {
        this.executeTrade(choice);
      }, duration);
    }
    ...
    executeTrade(choice) {
      ...
      const contractType = choice === 'EVEN' ? 'DIGITEVEN' : 'DIGITODD';
  ...
  window.SimSmartEvenBot = SimSmartEvenBot;
})();
