(() => {
  class SimSmartDifferBot {
    ...
    queueNextTrade() {
      ...
      this.tradeTimeout = setTimeout(() => this.executeTrade(market, digit), duration);
    }
    ...
    executeTrade(market, digit) {
      ...
      this.updateStats({ profit, win, market, digit, stake: this.currentStake });
      this.tradeInProgress = false;
    ...
    getStatsSnapshot(lastTrade) {
      ...
      return {
        ...
        digit: typeof this.currentDigit === 'number' ? `Differ ${this.currentDigit}` : '-',
  ...
  window.SimSmartDifferBot = SimSmartDifferBot;
})();
